...
if [ "$PARTNER_NODES" != "" ]; then
    ...
    DIFFERENT_UPSTREAMS=`PGPASSWORD=$REPLICATION_PASSWORD psql -h $CLUSTER_NODE_NETWORK_NAME -U $REPLICATION_USER -p $CURRENT_REPLICATION_PRIMARY_PORT $REPLICATION_DB  -tAc "SELECT upstream_node_id FROM $(get_repmgr_schema).$REPMGR_NODES_TABLE" | uniq | awk '{$1=$1};1' | grep -v -e '^[[:space:]]*$' | wc -l`
    ...
    read -d '' -ra NODES <<<`PGPASSWORD=$REPLICATION_PASSWORD psql -h $CLUSTER_NODE_NETWORK_NAME -U $REPLICATION_USER -p $CURRENT_REPLICATION_PRIMARY_PORT $REPLICATION_DB  -tAc "SELECT conninfo FROM $(get_repmgr_schema).$REPMGR_NODES_TABLE" | grep host | awk '{print $3}' | cut -d "=" -f2`
fi
...
for NODE in "${NODES[@]}"; do
    ...
    MASTER=`PGCONNECT_TIMEOUT=$CHECK_PGCONNECT_TIMEOUT PGPASSWORD=$REPLICATION_PASSWORD psql -h $NODE -U $REPLICATION_USER -p $CURRENT_REPLICATION_PRIMARY_PORT $REPLICATION_DB  -tAc "SELECT upstream_node_name FROM $(get_repmgr_schema).$REPMGR_SHOW_NODES_TABLE WHERE conninfo LIKE '%host=$NODE%' AND (upstream_node_name IS NOT NULL AND upstream_node_name <>'') AND active=true"`
    if [[ "$?" -ne "0" ]]; then
